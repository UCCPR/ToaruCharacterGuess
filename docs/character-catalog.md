...
`server/src/db/seeds/characterCatalog.json` 是人工整理的社区来源补充层。目前覆盖题库的 184 名角色，补充日文名、英文名、别名、多重阵营、组织、地点、首次登场和页面来源。它不会复制维基正文或图片。首次登场等仍不确定的字段必须在值中明确标注“待复核”，不能伪装成精确事实。

首次登场统一采用“角色实际登场”口径：角色必须作为能够确认身份的个体进入场景，单纯被提及姓名、出现在传闻或档案中、作为后台系统存在、由他人冒名，或仅在设定说明中出现，都不算首次登场。回忆、录像或再现影像只有在明确表现角色本人并让其参与场景时才计算。更早的名字或设定提及可以写进 `appearance.reference` 作为补充，但 `appearance.work` 和 `appearance.year` 必须对应首次实际登场；无法确认时标记为“待复核”。
...
当前累计题库为入门版 29 人、简单版 105 人、完整版 184 人。简单版不设固定人数：保留系列或外传的持续主角、长期核心同伴、主要阵营代表，以及能独立推动完整篇章的核心人物；仅在单个篇章承担局部战斗、说明或过场作用的人物留在完整版。调整角色时按这一标准独立判断，不要求一升一降。
