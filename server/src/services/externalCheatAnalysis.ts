...
import { getAllCachedPlayers, getPublicPlayerList } from './playerCache';
...
async function buildSnapshot(subject: AnalysisSubject, locale: AnalysisLocale, trigger: AnalysisTrigger) {
  ...
  const [
    ...
    getPublicPlayerList(),
    Promise.resolve(getAllCachedPlayers()),
    db('character_difficulties').select('character_id as playerId', 'difficulty_key as difficultyKey'),
  ]);
  ...
  const players = playerRows.map((player) => ({
    ...
    teamHistory: normalizeTeamHistory(player.team_history),
    age: Number(player.age),
    role: String(player.role),
    majorChampionships: Number(player.major_championships),
    majorAppearances: Number(player.major_appearances),
    isActive: Boolean(player.is_active),
    isEnabled: Boolean(player.is_enabled),
    difficulties: (difficultiesByPlayer.get(Number(player.id)) ?? []).sort(),
    createdAt: isoTimestamp(player.created_at),
  }));
