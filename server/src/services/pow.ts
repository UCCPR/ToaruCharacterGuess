...
const challengeCache = new Map<string, { stored: StoredChallenge; expiresAt: number }>();
...
export async function createChallenge(
  ...
  if (client) {
    ...
    const now = Date.now();
    for (const [key, value] of challengeCache) {
      if (value.expiresAt <= now) challengeCache.delete(key);
    }
    while (challengeCache.size >= 10_000) {
      challengeCache.delete(challengeCache.keys().next().value!);
    }
    challengeCache.set(id, {
      stored,
      expiresAt: now + config.powChallengeTtlSeconds * 1000,
    });
  }
  return {
    id,
...
export async function consumeAndVerifyChallenge(
  ...
  if (client) {
    ...
    raw = cached && cached.expiresAt > Date.now()
      ? JSON.stringify(cached.stored)
      : null;
  }
