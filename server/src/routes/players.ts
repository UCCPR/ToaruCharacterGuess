  ...
  asyncHandler(async (req, res) => {
    ...
    if (suggest) {
      return res.json(players.map((p) => ({
        id: p.id,
        nickname: p.nickname,
        localizedNames: p.localized_names,
      })));
    }
    res.json(
      players.map((p) => ({
        ...
        region: p.region,
        team: p.organizations?.map((organization) => organization.name).join('、') || '无所属',
        age: p.age,
        role: p.role,
        identities: p.identities?.map((identity) => identity.name) ?? [],
        majorChampionships: p.major_championships,
        majorAppearances: p.major_appearances,
        debutWork: p.debut_work,
