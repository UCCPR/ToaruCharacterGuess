...
router.use(['/players', '/player-change-submissions'], (_req, res) => {
  res.status(410).json({ code: 'CHARACTER_EDITING_DISABLED' });
});
...
function replayAnswer(target: Player) {
  return {
    ...
    localizedNames: target.localized_names,
    team: target.organizations?.map((organization) => organization.name).join('、') || '无所属',
    nationality: target.nationality,
    region: target.region,
    age: target.age,
    role: target.role,
    identities: target.identities?.map((identity) => identity.name) ?? [],
    majorChampionships: target.major_championships,
    majorAppearances: target.major_appearances,
    debutWork: target.debut_work,
  ...
  asyncHandler(async (req, res) => {
    ...
    if (type === 'single') {
      const rows = await db('games as g')
        .join('characters as p', 'p.id', 'g.target_player_id')
        .where('g.user_id', id)
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (parsed.type === 'single') {
      const rows = await db('games as g').join('characters as p', 'p.id', 'g.target_player_id')
        ...
        .select('g.id', 'g.mode', 'g.status', 'g.guess_count as guessCount', 'g.finished_at as finishedAt', 'p.canonical_name_zh as answer');
