...
import { ensureCharacterCatalogSchema } from './characterCatalogSchema';
...
export async function ensureSchema(instance: Knex = db): Promise<void> {
  ...
  await ensureCharacterCatalogSchema(instance);
  ...
  if (!(await instance.schema.hasTable('games'))) {
    await instance.schema.createTable('games', (t) => {
      ...
      t.integer('target_player_id').notNullable().references('id').inTable('characters');
