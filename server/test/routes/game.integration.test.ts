...
import { shouldPersistSingleSettlement } from '../../src/services/singleSettlementLimit';
import { loadSingleGame } from '../../src/services/singleGameStore';
...
describe('single-player settlement soft limit', () => {
  ...
  it('resumes without recording a discarded target and avoids an immediate repeat', async () => {
    ...
    const firstGame = await loadSingleGame(first.data.gameId, identityKey);
    const firstTarget = firstGame!.targetPlayerId;
    try {
      const resumed = await post('/api/game/start', cookie, { mode: 'beginner' });
      expect(resumed.data.gameId).toBe(first.data.gameId);
      if (redis()) expect(await redis()!.zCard(historyKey)).toBe(1);
      ...
      const secondGame = await loadSingleGame(second.data.gameId, identityKey);
      const secondTarget = secondGame!.targetPlayerId;
  ...
  it('persists accepted guess times relative to the game start', async () => {
    ...
    const started = await post('/api/game/start', cookie, { mode: 'beginner' });
    expect(started.response.status).toBe(200);
    const activeGame = await loadSingleGame(started.data.gameId, `g:${guestKey}`);
    const targetPlayerId = activeGame!.targetPlayerId;
    try {
