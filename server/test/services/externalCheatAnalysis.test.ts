...
import { HttpError } from '../../src/middleware/common';
import { initRedis } from '../../src/redis';
import { getAllCachedPlayers, initPlayerCache } from '../../src/services/playerCache';
import { externalAnalysisResponseSchema, requestExternalCheatAnalysis } from '../../src/services/externalCheatAnalysis';
...
beforeAll(async () => {
  ...
  const [target, guess] = await db('characters').select('id').orderBy('id').limit(2);
...
describe('external cheat analysis', () => {
  ...
  it('sends an anonymous bearer-authenticated snapshot and accepts a validated response', async () => {
    ...
    const targetPlayer = cachedPlayers.find((player) => player.id === targetPlayerId)!;
    expect(request.playerPool.revision).toEqual(expect.any(String));
    expect(request.playerPool.players).toHaveLength(cachedPlayers.length);
    expect(request.playerPool.players).toEqual(expect.arrayContaining([expect.objectContaining({
      id: targetPlayerId,
      nickname: String(targetPlayer.nickname),
      nationality: String(targetPlayer.nationality),
      difficulties: [...targetPlayer.difficulties].sort(),
    })]));
