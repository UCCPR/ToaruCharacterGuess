...
describe('multiplayer socket integration', () => {
    ...
    try {
      ...
      for (let guessIndex = 0; guessIndex < wrongGuesses.length; guessIndex += 1) {
        ...
        for (const applied of await Promise.all(appliedEvents)) {
          ...
          expect(applied.feedback.attributes.region).toHaveProperty('value');
        }
  ...
  it('validates and applies custom room guess limits, intervals, and round duration', async () => {
    ...
    try {
      ...
      const wrongGuesses = await db('characters as character')
        .join('character_game_profiles as profile', 'profile.character_id', 'character.id')
        .whereNot('character.id', active!.targetPlayerId)
        .where('profile.is_enabled', true)
        .select('character.id')
        .limit(2);
  ...
  it('hides opponent guess details from players but not spectators', async () => {
    ...
    try {
      ...
      const [wrongGuess] = await db('characters')
  ...
  it('uses incremental guesses and reloads the Redis script after SCRIPT FLUSH', async () => {
    ...
    try {
      ...
      const wrongGuesses = await db('characters as character')
        .join('character_game_profiles as profile', 'profile.character_id', 'character.id')
        .whereNot('character.id', stored.targetPlayerId)
        .where('profile.is_enabled', true)
        .select('character.id')
        .limit(2);
