...
```text
ghcr.io/uccpr/toarucharacterguess
```

运行镜像基于 distroless Node.js，仅包含生产 Node 依赖、编译后的服务端
JavaScript、编译后的前端资源以及输出到 `server/dist` 的角色目录种子。镜像中
...
Compose 会先运行一次性的 `migrate` 服务：它负责创建或更新表和索引，并同步
内置的规范化角色目录。迁移未成功退出前，两个应用实例都
...
```text
IMAGE=ghcr.io/uccpr/toarucharacterguess:sha-0123456
```
