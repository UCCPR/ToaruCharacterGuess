...
describe('AnswerOverlay', () => {
  it('exposes dialog semantics used by keyboard focus guards', () => {
    ...
    expect(screen.getByText('魔法禁书目录（旧约）')).toBeInTheDocument();
  });
  ...
  it('distinguishes canonically unknown from no-gender entities', () => {
    renderWithProviders(<PlayerInfoTable answer={{ ...answer, majorChampionships: 2 }} />);
    expect(screen.getByText('未知（原作未确认）')).toBeInTheDocument();

    renderWithProviders(<PlayerInfoTable answer={{ ...answer, majorChampionships: 3 }} />);
    expect(screen.getByText('无性别')).toBeInTheDocument();
  });
});
