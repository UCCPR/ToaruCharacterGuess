...
import { Globe, Crosshair, Calendar, Shield, Trophy, Layers3, BookOpen } from 'lucide-react';
...
export function PlayerInfoTable({ answer }: { answer: AnswerInfo }) {
  ...
  const lore = (value: string) => localizeLoreValue(t, value, i18n.language);
  const rows: [ReactNode, string, ReactNode][] = [
    [<Shield size={14} key="i" />, t('player.team'), answer.team ? lore(answer.team) : '-'],
    [<Globe size={14} key="i" />, t('player.nationality'), `${lore(answer.nationality)}${answer.region ? ` · ${lore(answer.region)}` : ''}`],
    [<Crosshair size={14} key="i" />, t('player.role'), answer.identities?.length
      ? answer.identities.map(lore).join(i18n.language.startsWith('en') ? ', ' : '、')
      : answer.role ? lore(answer.role) : '-'],
    [<Trophy size={14} key="i" />, t('player.majorChampionships'), t(genderLabelKey(answer.majorChampionships))],
    [<BookOpen size={14} key="i" />, t('player.debutWork'), answer.debutWork ? lore(answer.debutWork) : '-'],
          ...
          {answer && (
            <>
              <p className="answer-name">
                {answer.localizedNames?.[i18n.language.split('-')[0] as 'zh' | 'en' | 'ja']
                  || localizedPlayerName(answer.id, answer.nickname, i18n.language)}
              </p>
