...
type Suggestion = PlayerSuggestion;
...
interface Props {
  ...
  placeholder?: string;
  buttonText?: string;
  /** Optional in-browser catalog for builds that do not have a player-list API. */
  playerList?: Suggestion[];
}
...
export default function GuessInputBar({
  ...
  const visibleName = useCallback((item: Suggestion) => {
    const locale = i18n.language.split('-')[0] as 'zh' | 'en' | 'ja';
    return item.localizedNames?.[locale] || localizedPlayerName(item.id, item.nickname, i18n.language);
  }, [i18n.language]);
      ...
      <form className="input-bar" onSubmit={submit}>
        <input
          ...
          onFocus={() => {
            ...
            if (!playerList) void getPlayerList().catch(() => undefined);
            onFocusChange?.(true);
          }}
          onBlur={() => {
            focused.current = false;
            ...
            if (e.key === 'ArrowDown') {
              ...
              const nextActive = tabCycling.current
                ...
                : e.shiftKey
                  ...
                  : active;
              const completed = visibleName(items[nextActive]);
