...
function AnswerSection({ answer }: { answer: PlayerInfo }) {
  ...
  const answerName = answer.localizedNames?.[i18n.language.split('-')[0] as 'zh' | 'en' | 'ja']
    ?? (answer.id ? localizedPlayerName(answer.id, answer.nickname, i18n.language) : answer.nickname);
  return (
    <section className="replay-answer" aria-label={t('replay.answerLabel')}>
      <h3>{t('replay.correctAnswer', { name: answerName })}</h3>
      <PlayerInfoTable
        answer={{
          id: answer.id,
          nickname: answer.nickname,
          localizedNames: answer.localizedNames,
          team: answer.team,
  ...
  return (
    <>
                ...
                {replay.type === 'single'
                  ...
                  : t('replay.multiSummary', {
                    ...
                    opponent: replay.gameMode === 'relay2v2'
                      ...
                      : localizeDisplayName(t, replay.opponent?.displayId ?? t('stats.unknownOpponent')),
                    ...
                    score: replay.gameMode === 'relay'
                      ...
                      : replay.participants?.length
                        ? replay.participants.map((participant) => `${participant.isMe ? t('common.me') : localizeDisplayName(t, participant.displayId)} ${participant.score}`).join(' · ')
                  ...
                  <section className="replay-round" key={activeRound.round}>
                    ...
                    {replay.gameMode === 'relay2v2' ? (
                              ...
                              {teamGuesses.length ? <>
                                <GuessBoard
                                  ...
                                  rowAnnotations={teamGuesses.map((guess) => {
                                    const actorLabel = guess.actor === 'me'
                                      ? t('common.me')
                                      : localizeDisplayName(t, guess.actorDisplayId ?? '-');
                                    return {
                                      content: actorLabel,
                                      title: actorLabel,
                            ...
                            rowAnnotations={activeRound.sharedGuesses.map((guess) => {
                              const label = guess.actor === 'me'
                                ...
                                : guess.actor === 'opponent'
                                  ? localizeDisplayName(t, replay.opponent?.displayId ?? '-')
                                  : localizeDisplayName(t, guess.actorDisplayId ?? '-');
                              return {
                                content: label,
                                title: label,
                      ...
                      <div className="replay-multi-player-layout">
                        <div className="replay-participant-list" role="tablist" aria-label={t('multi.otherPlayers')}>
                          {multiPlayerRounds.map(({ participantId, participant }) => (
                            ...
                            >
                              <span>{participant?.isMe ? t('common.me') : localizeDisplayName(t, participant?.displayId ?? '-')}</span>
                        ...
                        <div className="replay-side">
                          <h4><User size={15} />{selectedRoundPlayer.participant?.isMe
                            ? t('replay.mySide')
                            : localizeDisplayName(t, selectedRoundPlayer.participant?.displayId ?? '-')}</h4>
                      ...
                      <div className="replay-side">
                        <h4>
                          <Swords size={15} />
                          <span>{localizeDisplayName(t, replay.opponent?.displayId ?? t('stats.unknownOpponent'))}</span>
                          {onViewOpponentStats && (
                            <button
                              type="button"
                              className="player-stats-trigger"
                              aria-label={t('multi.viewPlayerStats', { player: localizeDisplayName(t, replay.opponent?.displayId ?? '') })}
      ...
      {opponentStatsOpen && opponentStats && replay.type === 'multi' && (
        <PlayerStatsDialog
          view={{ displayId: localizeDisplayName(t, replay.opponent?.displayId ?? t('stats.unknownOpponent')), stats: opponentStats }}
