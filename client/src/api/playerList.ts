...
/** Supplies the catalog for a browser-only build without contacting the API. */
export function setPlayerListSnapshot(players: PlayerSuggestion[], version = 'static'): void {
  memory = { version, players };
  validatedAt = performance.now();
  publish(players);
}
...
export function localizedPlayerName(
  ...
  const player = playerId == null ? undefined : memory?.players.find((item) => item.id === playerId);
  const locale = language.split('-')[0] as 'zh' | 'en' | 'ja';
  return player?.localizedNames?.[locale] || fallback;
}
...
export function searchPlayerList(players: PlayerSuggestion[], query: string): PlayerSuggestion[] {
  ...
  return players
    .map((player) => ({
      player,
      score: Math.min(
        matchScore(player.nickname, normalized),
        matchScore(player.localizedNames?.en ?? '', normalized),
        matchScore(player.localizedNames?.ja ?? '', normalized),
        ...((player.searchTerms ?? []).map((value) => matchScore(value, normalized)))
      ),
    }))
