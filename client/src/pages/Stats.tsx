...
export default function Stats() {
  ...
  const multiColumns: Column<MultiReplayItem>[] = [
    ...
    { key: 'opponent', title: t('stats.matchup'), render: (game) => game.gameMode === 'relay2v2'
      ...
      ? `${t('common.me')} / ${game.participants.map((participant) => localizeDisplayName(t, participant.displayId)).join(' / ')}`
      : `${t('common.me')} / ${localizeDisplayName(t, game.opponent?.displayId ?? t('stats.unknownOpponent'))}` },
                  ...
                  {item.type === 'single' ? (
                    ...
                    <>
                      <span>{t('stats.matchup')} <strong>{item.gameMode === 'relay2v2'
                        ...
                        ? `${t('common.me')} / ${item.participants.map((participant) => localizeDisplayName(t, participant.displayId)).join(' / ')}`
                        : `${t('common.me')} / ${localizeDisplayName(t, item.opponent?.displayId ?? t('stats.unknownOpponent'))}`}</strong></span>
